...
def get_market_data():
    try:
        ...
        liq = float((m2.iloc[-1] - m2.iloc[-2]) / m2.iloc[-2])
...
def check_radar(ticker, name):
    ...
    # Extração blindada
    vals = df['Close'].tail(200).values
    close_val = float(vals[-1])
    mean_val = float(np.mean(vals))
    std_val = float(np.std(vals))
    z_score = (close_val - mean_val) / std_val if std_val != 0 else 0.0
    
    liq, y10 = get_market_data()
    risk = get_sentiment()
    
    msg = (f"--- Radar de Elite: {name} ---\n"
           f"Risco: {float(risk):.2f} | Z-Score: {float(z_score):.2f} | "
           f"Liq: {float(liq):.4f} | Yield: {float(y10):.2f}%")
